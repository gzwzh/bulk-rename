...
class UpdateWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle(_("updater.title"))
        ...
        self.title_label = QLabel(_("updater.upgrading"))
        ...
        self.status_label = QLabel(_("updater.ready"))
...
def update_worker(args, signals):
    try:
        ...
        if args.pid:
            signals.status.emit(_("updater.wait_exit"))
        ...
        zip_path = args.zip
        if args.url:
            signals.status.emit(_("updater.downloading"))
            try:
                ...
                if args.hash:
                    ...
                    if actual_hash.lower() != args.hash.lower():
                        ...
                        signals.status.emit(_("updater.verify_fail"))
                ...
                signals.status.emit(_("updater.fail").format(error=str(e)))
                signals.finished.emit(False)
                return
        
        # 3. Install update
        signals.status.emit(_("updater.unpacking"))
        ...
        if not success:
            signals.status.emit(_("updater.install_fail"))
            signals.finished.emit(False)
            return

        signals.progress.emit(95)
        signals.status.emit(_("updater.cleaning"))
        ...
        if os.path.exists(exe_path):
            try:
                ...
                signals.status.emit(_("updater.restart_fail"))
                signals.finished.emit(False)
        else:
            log(f"Executable not found: {exe_path}")
            signals.status.emit(_("updater.not_found"))
            signals.finished.emit(False)
            
    except Exception as e:
        log(f"Critical error in worker: {traceback.format_exc()}")
        signals.status.emit(_("updater.error"))
        signals.finished.emit(False)
