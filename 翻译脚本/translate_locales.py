...
DEFAULT_MODEL = "qwen-coder-turbo-0919"
...
PLACEHOLDER_RE = re.compile(r"\{\{\s*[^{}]+\s*\}\}|\{[^{}]+\}|\(&[A-Z]\)")
...
def flatten_object(obj: dict, prefix: str = "", acc: Dict[str, str] | None = None) -> Dict[str, str]:
    ...
    for key, value in obj.items():
        ...
        if isinstance(value, list):
            # Handle lists by using index suffixes
            for idx, item in enumerate(value):
                if not isinstance(item, str):
                    raise ValueError(f"Only string leaf values (or list of strings) are supported. Problem key: {path_key}[{idx}]")
                acc[f"{path_key}.{idx}"] = item
            continue
...
def unflatten_object(flat: Dict[str, str]) -> dict:
    result: dict = {}
    # First pass: identify keys that should be lists
    list_keys: Dict[str, Dict[int, str]] = {}
    
    for path_key, value in flat.items():
        keys = path_key.split(".")
        # Check if the last part is a digit (list index)
        if len(keys) > 1 and keys[-1].isdigit():
            parent_key = ".".join(keys[:-1])
            index = int(keys[-1])
            if parent_key not in list_keys:
                list_keys[parent_key] = {}
            list_keys[parent_key][index] = value
        else:
            # Standard string key
            cursor = result
            for part in keys[:-1]:
    ...
    # Second pass: inject lists into the result
    for parent_key, index_map in list_keys.items():
        # Sort by index and create list
        sorted_indices = sorted(index_map.keys())
        # Ensure indices are contiguous starting from 0 (simple check)
        list_value = [index_map[i] for i in sorted_indices]
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--source-lang-code", default="zh_CN", help="Source language code (default: zh_CN)")
